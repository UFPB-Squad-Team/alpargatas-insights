...
const schoolSchema: Schema = new Schema({
    ...
    dependenciaAdm: { type: String, enum: Object.values(dependenciaAdministrativa), required: true },
    tipoLocalizacao: { type: String, enum: Object.values(tipoLocalizacao), required: true },
    localizacao: { 
        type: { type: String, required: true },
        coordinates: { type: [Number], required: true, validate: { validator: (coords: [number, number]) => coords.length === 2, message: 'Coordinates need two numbers' } }
    },
