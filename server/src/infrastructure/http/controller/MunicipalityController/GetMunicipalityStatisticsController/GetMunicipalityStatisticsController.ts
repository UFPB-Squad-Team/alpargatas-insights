...
export class GetMunicipalityStatisticsController{
    ...
    async getStatistics(req: Request, res: Response){
        ...
        const paramSchema = z.object({
            municipioIdIbge: z.string().trim().length(7, { message: "Need 7 caracteres" })
        })

        const { municipioIdIbge } = paramSchema.parse(req.params)

        const municipality = await this.getMunicipalityStatisticsUseCase.execute({ municipioIdIbge })
