...
export class MoongoseSchoolRepository implements ISchoolRepository{
    ...
    async findByDepAdm(dependenciaAdm: dependenciaAdministrativa): Promise<School[]> {
       const pipeline = [
            {
    ...
    async findSearchByTerm(term: string, page: number, limit: number): Promise<{ schools: School[]; total: number; page: number; currentPage: number; }> {
        ...
        const [school, total] = await Promise.all([
            SchoolModel.find(query).skip((page-1) * limit).limit(limit).lean(),
    ...
    async findAllForMap(): Promise<Pick<School, "id" | "escolaNome" | "localizacao" | "scoreRisco">[]> {
        const pipeline = [
            {
                $project: {
                    _id: 0,
    ...
    async save({ municipioIdIbge, escolaIdInep, escolaNome, municipioNome, estadoSigla, dependenciaAdm,tipoLocalizacao, localizacao, indicadores, scoreRisco, infraestrutura }: School): Promise<void> {
        await SchoolModel.create({ municipioIdIbge, escolaIdInep, escolaNome, municipioNome, estadoSigla, dependenciaAdm, tipoLocalizacao, localizacao, indicadores, scoreRisco, infraestrutura })
    }
