...
export class NeedValidator {
  static validate(props: NeedProps) {
    ...
    if (!props.description || props.description.trim() === '') {
      throw new AppError(
        'Description is required and need at least 1 character',
      );
    }

    if (!Object.values(NeedType).includes(props.type)) {
      throw new AppError(
        `Invalid type, must be one of this ${Object.values(NeedType).join('')}`,
      );
    }

    if (!Object.values(NeedStatus).includes(props.status)) {
      throw new AppError(
        `Invalid type, must be one of this ${Object.values(NeedStatus).join('')}`,
      );
    }

    if (!Object.values(SubmitterType).includes(props.submitterType)) {
      throw new AppError(
        `Invalid type, must be one of this ${Object.values(SubmitterType).join('')}`,
      );
    }
  }
}
