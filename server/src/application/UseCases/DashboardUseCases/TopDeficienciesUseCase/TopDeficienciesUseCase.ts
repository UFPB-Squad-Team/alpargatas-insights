...
export class TopDeficienciesUseCase{
    ...
    async execute(): Promise<TopDeficienciesReturnDTO>{
        ...
        .filter((school) => school.scoreRisco >= this.HIGH_RISK_THRESHOLD);

        schoolsWithHighInfraestructureRisk.forEach((school) => {
            Object.entries(school.infraestrutura).forEach(([item, available]) => {
                if(available === false){
                    const formattedField = 'Falta de' + ' ' + item.charAt(0).toLocaleUpperCase() + item.slice(1).toLowerCase()

                    deficienciesCount[formattedField] = (deficienciesCount[formattedField] || 0) + 1
                }
            })
        })

        const topDeficiencies = Object.entries(deficienciesCount).map(([deficit, schools]) => ({ deficit, schools })).sort((a,b) => b.schools - a.schools)
