...
export class GetMunicipalityStatisticsUseCase{
    ...
    async execute({ municipioIdIbge }: GetMunicipalityStatisticsDTO): Promise<ReturnStatisticsFromMunicipalityDTO>{
        ...
        if(schools.length === 0){
            throw new AppError("No have schools registered in this municipality", HTTPSTATUS.NOT_FOUND)
        }

        const totalSchools = schools.length

        const municipalityTotalRisk = schools.reduce<number>((sum, school) => sum + school.scoreRisco, 0)
