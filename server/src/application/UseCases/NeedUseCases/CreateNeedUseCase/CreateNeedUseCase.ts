...
export class CreateNeedUseCase{
   ...
   */
    async execute({ title, description, type, submitterType, submitterContact, location, status }: CreateNeedInputDTO): Promise<CreateNeedOutputDTO>{

        const need = new Need({ title, description, type, submitterType, submitterContact, location, status })
