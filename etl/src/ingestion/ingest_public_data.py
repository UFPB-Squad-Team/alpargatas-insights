...
def _ingest_source(
    source_name: str, source_config: dict, s3_client, bucket_name: str, raw_folder: str
):
    ...
    try:
        ...
        logging.error(
            f"FALHA: Erro inesperado para a fonte '{source_name}': {e}", exc_info=True
        )
    return False
...
def run():
    ...
    try:
        ...
        logging.error(
            f"Não foi possível inicializar o cliente S3. Verifique suas credenciais no .env. Erro: {e}"
        )
        return
    ...
    for source_name, source_config in sources_to_ingest.items():
        if _ingest_source(
            source_name, source_config, s3_client, bucket_name, raw_folder
        ):
            success_count += 1

    logging.info("--- JOB DE INGESTÃO FINALIZADO ---")
    logging.info(
        f"Resumo: {success_count} de {len(sources_to_ingest)} fontes ingeridas com sucesso."
    )
