...
class MongoLoader:
    ...
    def bulk_upsert(self, operations: List[UpdateOne], collection_name:str):
        ...
        """
        if not operations:
            logging.waring("A lista de operações está vazia. Nenhum dado será carregado")
            return
        
        logging.info(f"Carregando {len(operations)} registros para a coleção '{collection_name}'")
        try:
            ...
            logging.error(f"Um erro inesperado ocorreu durante o carregamento: {e}", exc_info=True)
            raise
