...
def _extract_and_transform_inse(
    ...
    df_inse_bruto.rename(
        columns={"ID_ESCOLA": "escolaIdInep", "MEDIA_INSE": "inse_valor"}, inplace=True
    )

    logging.info(
        "Transformação concluída. DataFrame com %s linhas.", len(df_inse_bruto)
    )
...
def run():
    ...
    try:
        ...
        logging.info(
            f"Salvando o arquivo Parquet na camada 'intermediate' do S3 em: {output_s3_path}"
        )
        df_inse_transformed.to_parquet(
            output_s3_path, index=False, storage_options=storage_options
        )

        logging.info(
            "--- PIPELINE DO INSE CONCLUÍDO COM SUCESSO. DADOS SALVOS NO S3. ---"
        )
    except Exception as e:
