...
def _process_extracted_data(df_dtb: pd.DataFrame, df_ia: pd.DataFrame, df_ideb: pd.DataFrame, config: dict) -> tuple:
    ...
    df_ia_processed['nprojetos'] = pd.to_numeric(df_ia_processed['nprojetos'], errors='coerce').fillna(0)
    df_ia_processed['nbeneficiados'] = pd.to_numeric(df_ia_processed['nbeneficiados'], errors='coerce').fillna(0)
    df_ia_processed.dropna(subset=["ds_mun", "sg_uf"], inplace=True)
    df_ia_processed = df_ia_processed[df_ia_processed["sg_uf"].isin(config['valid_ufs'])]
...
def _create_municipal_summary(df_dtb: pd.DataFrame, df_ia: pd.DataFrame, df_ideb: pd.DataFrame) -> pd.DataFrame:
...
def _enrich_school_data(df_escolas: pd.DataFrame, df_municipal: pd.DataFrame, weights: dict) -> pd.DataFrame:
...
def run(df_dtb: pd.DataFrame, df_ia: pd.DataFrame, df_ideb: pd.DataFrame):
    ...
    df_dtb_p, df_ia_p, df_ideb_p = _process_extracted_data(df_dtb, df_ia, df_ideb, transform_config)
