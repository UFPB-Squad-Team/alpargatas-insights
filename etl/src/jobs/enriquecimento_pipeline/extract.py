...
def _extract_dtb(
    bucket_name: str, raw_folder: str, source_config: dict
) -> pd.DataFrame:
...
def _extract_ideb(
    ...
    numeric_cols = [
        col for col in df.columns if col.startswith("ideb_") or "NOTA" in col
    ]
...
def _extract_ia_projects(
    bucket_name: str, raw_folder: str, ia_config: dict, storage_options: dict
) -> pd.DataFrame:
    ...
    all_sheets = pd.read_excel(
        ia_path, sheet_name=None, storage_options=storage_options
    )
    ...
    df_consolidado["nprojetos"] = (
        pd.to_numeric(df_consolidado["nprojetos"], errors="coerce")
        .fillna(0)
        .astype(int)
    )
    df_consolidado["nbeneficiados"] = (
        pd.to_numeric(df_consolidado["nbeneficiados"], errors="coerce")
        .fillna(0)
        .astype(int)
    )

    logging.info(
        f"Dados de Projetos IA extraídos com sucesso: {len(df_consolidado)} registros."
    )
    return df_consolidado


def _extract_municipios(
    bucket_name: str, raw_folder: str, storage_options: dict
) -> pd.DataFrame:
...
def run() -> None:
    ...
    ia_extract_config = config["professor_pipeline"]["extract"][
    ...
    try:
        ...
        df_ia = _extract_ia_projects(
            bucket_name, raw_folder, ia_extract_config, storage_options
        )
        ...
        df_dtb.to_parquet(
            f"s3://{bucket_name}/{paths_config['intermediate_professor_dtb']}",
            storage_options=storage_options,
        )
        df_ideb.to_parquet(
            f"s3://{bucket_name}/{paths_config['intermediate_professor_ideb']}",
            storage_options=storage_options,
        )
        df_ia.to_parquet(
            f"s3://{bucket_name}/{paths_config['intermediate_professor_ia']}",
            storage_options=storage_options,
        )
        df_municipios.to_parquet(
            f"s3://{bucket_name}/{paths_config['intermediate_professor_municipios']}",
            storage_options=storage_options,
        )
        ...
        logging.error(
            f"Falha na execução da extração do pipeline do professor: {e}",
            exc_info=True,
        )
        raise
