...
def run() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    df_dtb = pd.read_excel(BASE_DIR / paths['raw_professor_dtb'], skiprows=6, usecols=["UF", "Nome_UF", "Código Município Completo", "Nome_Município"])
    df_dtb.columns = ["id_uf", "ds_uf", "id_mundv", "ds_mun"]
    df_dtb = df_dtb.drop_duplicates(subset=["id_mundv"])
    logging.info(f"Dados do DTB extraídos com sucesso: {len(df_dtb)} municípios únicos.")
