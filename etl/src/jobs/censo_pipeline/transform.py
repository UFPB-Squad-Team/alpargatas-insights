...
def _calculate_municipal_profile(df_censo_renda: pd.DataFrame) -> pd.DataFrame:
    ...
    df_valid_renda = df_censo_renda[df_censo_renda['renda_media_domiciliar_setor'] > 0].copy()
    df_valid_renda['municipioIdIbge'] = df_valid_renda['id_setor_censitario'].str[:7].astype(int)
    
    socioeconomic_cols = [col for col in df_valid_renda.columns if col.startswith(('total_', 'domicilios_', 'renda_'))]
    rename_map = {col: f"mun_{col}" for col in socioeconomic_cols}
    
    df_municipio_profile = df_valid_renda.groupby('municipioIdIbge')[socioeconomic_cols].mean().rename(columns=rename_map)
    return df_municipio_profile.reset_index()


# Em: src/jobs/censo_pipeline/transform_final.py

def _calculate_neighborhood_profile(gdf_escolas: gpd.GeoDataFrame, gdf_malha_completa: gpd.GeoDataFrame) -> pd.DataFrame:
    """
    ...
    socioeconomic_cols = [col for col in gdf_malha_completa.columns if col.startswith(('total_', 'domicilios_', 'renda_'))]
    for col in socioeconomic_cols:
        intersection_gdf[f'weighted_{col}'] = intersection_gdf[col].fillna(0) * intersection_gdf['intersection_area']
        
    grouped = intersection_gdf.groupby('escolaIdInep').agg(
        total_intersection_area=('intersection_area', 'sum'),
        **{f'sum_weighted_{col}': (f'weighted_{col}', 'sum') for col in socioeconomic_cols}
    )
    ...
    for col in socioeconomic_cols:
        ...
        df_neighborhood[f'viz_{col}'] = np.divide(numerator, denominator, out=np.zeros_like(numerator, dtype=float), where=(denominator != 0))

    # --- ETAPA 2: Imputação com Mediana dos Vizinhos (para quem ficou com zero) ---
    escolas_sem_dados_ids = df_neighborhood[df_neighborhood[f'viz_{socioeconomic_cols[0]}'] == 0].index
    logging.info(f"Encontradas {len(escolas_sem_dados_ids)} escolas em 'buracos de dados'. Iniciando imputação com vizinhos mais próximos...")
    
    if not escolas_sem_dados_ids.empty:
        gdf_good_sectors = gdf_malha_completa[gdf_malha_completa['renda_media_domiciliar_setor'] > 0].copy()
        
        # --- CORREÇÃO APLICADA AQUI ---
        # 1. Garante que ambos os GeoDataFrames para o cálculo de distância estejam no CRS projetado (em metros)
        gdf_escolas_dist_proj = gdf_escolas[gdf_escolas['escolaIdInep'].isin(escolas_sem_dados_ids)].to_crs(target_crs)
        ...
        for index, escola in gdf_escolas_dist_proj.iterrows():
            ...
            median_values = gdf_good_sectors.loc[nearest_sectors_idx, socioeconomic_cols].median()
...
def _structure_final_output(df: pd.DataFrame) -> pd.DataFrame:
    ...
    final_schema_order = [
        ...
        "contextoMunicipal", "contextoVizinhança"
    ]
    
    df_structured = df.drop(columns=municipal_cols + neighborhood_cols)
    return df_structured[[col for col in final_schema_order if col in df_structured.columns]]
...
def run():
    ...
    try:
        ...
        df_escolas = pd.read_parquet(f"s3://{s3_config['bucket_name']}/{paths_config['processed_escolas']}", storage_options=storage_options)
        df_censo_renda = pd.read_parquet(f"s3://{s3_config['bucket_name']}/{paths_config['intermediate_malha_setorial']}", storage_options=storage_options)
        gdf_malha_setores = gpd.read_file(f"s3://{s3_config['bucket_name']}/{paths_config['raw_malha_setorial']}", storage_options=storage_options, engine="fiona")

        # Preparação dos dados geoespaciais e de escolas
        id_setor_col_name = next((col for col in gdf_malha_setores.columns if "CD_SETOR" in col.upper()))
        gdf_malha_setores['id_setor_censitario'] = gdf_malha_setores[id_setor_col_name].astype(str)
        df_censo_renda['id_setor_censitario'] = df_censo_renda['id_setor_censitario'].astype(str)
        gdf_malha_completa = gdf_malha_setores.merge(df_censo_renda, on='id_setor_censitario', how='left')

        coords = df_escolas['localizacao'].apply(lambda loc: loc.get('coordinates', [None, None]))
        gdf_escolas = gpd.GeoDataFrame(
            df_escolas, 
            geometry=gpd.points_from_xy(coords.apply(lambda c: c[0]), coords.apply(lambda c: c[1])), 
            crs="EPSG:4326"
        ...
        df_neighborhood_profile = _calculate_neighborhood_profile(gdf_escolas, gdf_malha_completa)
        
        # 3. Merge Final de tudo
        logging.info("Unindo todos os perfis calculados...")
        df_merged_1 = pd.merge(df_escolas, df_municipal_profile, on='municipioIdIbge', how='left')
        df_final_merged = pd.merge(df_merged_1, df_neighborhood_profile, on='escolaIdInep', how='left')
        ...
        logging.info(f"--- PIPELINE FINALIZADA COM SUCESSO! ---")
