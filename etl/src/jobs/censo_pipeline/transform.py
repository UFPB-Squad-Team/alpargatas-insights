...
def _impute_missing_with_knn_median(
    ...
    escolas_sem_dados_ids = df_imputed[df_imputed[f"viz_{socioeconomic_cols[0]}"] <= 0].index
    
    logging.info(f"Encontradas {len(escolas_sem_dados_ids)} escolas para imputação. Iniciando busca por vizinhos...")
    ...
    gdf_escolas_to_impute_proj = gdf_escolas[gdf_escolas["escolaIdInep"].isin(escolas_sem_dados_ids)].to_crs(target_crs)
...
def _calculate_neighborhood_profile(gdf_escolas: gpd.GeoDataFrame, gdf_malha_completa: gpd.GeoDataFrame) -> pd.DataFrame:
    ...
    df_direct_avg = _calculate_direct_weighted_average(gdf_escolas_proj, gdf_malha_proj, socioeconomic_cols)
    
    # 2. Imputa os valores faltantes
    df_final_profile = _impute_missing_with_knn_median(df_direct_avg, gdf_escolas, gdf_malha_completa, socioeconomic_cols)
...
def run():
    ...
    """
    logging.info("--- INICIANDO PIPELINE DE TRANSFORMAÇÃO: RENDA CENSO (SETOR CENSITÁRIO) ---")
    try:
        config = load_config()
        s3_config = config["s3"]
