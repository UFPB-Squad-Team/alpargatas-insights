...
def _define_schema() -> pa.DataFrameSchema:
    ...
    return pa.DataFrameSchema(
        columns={
            ...
            "estadoSigla": pa.Column(str, checks=pa.Check.str_length(min_value=2, max_value=2)),
            "dependenciaAdm": pa.Column(str, checks=pa.Check.isin(["Federal", "Estadual", "Municipal"])),
            "tipoLocalizacao": pa.Column(str, checks=pa.Check.isin(["Urbana", "Rural"])),
            ...
            "scoreRisco": pa.Column(float, checks=pa.Check.in_range(min_value=0.0, max_value=1.0)),
        },
        strict=True,
        ordered=True,
    )
...
def run():
    ...
    logging.info("--- INICIANDO JOB DE VALIDAÇÃO DE DADOS (PIPELINE DE ESCOLAS) DO S3 ---")
    ...
    try:
        ...
        df_processed = pd.read_parquet(processed_s3_path, storage_options=storage_options)
        ...
        logging.info(" SUCESSO: Os dados processados são válidos e seguem o contrato de qualidade.")

    except FileNotFoundError:
        logging.error(f"ERRO: Arquivo de dados processados não encontrado em '{processed_s3_path}'.")
        sys.exit(1)
