...
def run():
    ...
    try:
        ...
        df_mapped = _map_categorical_values(
            df_jittered, transform_config["categorical_maps"]
        )  # 2. Usa o resultado com jitter
        df_infra_processed = _process_infra_columns(df_mapped)
        df_structured = _structure_for_nosql(
            df_infra_processed
        )  # 3. Esta função agora usará as coordenadas com jitter
