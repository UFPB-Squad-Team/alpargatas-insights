...
def calculate_advanced_infra_percent(df_paraiba, infra_avancada):
    ...
    for codigo, nome in infra_avancada.items():
        ...
        porcentagem_escolas_com_infra_avancada.append(
            {"Item": nome, "Com (%)": percentual_com_infra, "Total": total}
        )

    df_resultado_porcentagem_escolas_com_infra_avancada = pd.DataFrame(
        porcentagem_escolas_com_infra_avancada
