...
def extract_data_from_local_zip(zip_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        logging.info(f"Extração finalizada. Encontrados {len(df_paraiba)} registros para a Paraíba.")
        return df_paraiba

    except FileNotFoundError:
        logging.error(f"Erro: O arquivo ZIP não foi encontrado no caminho '{zip_path}'. Verifique o Dockerfile.")
    except zipfile.BadZipFile:
        logging.error("Erro de descompactação: O arquivo não é um ZIP válido.")
    except Exception as e:
        logging.error(f"Ocorreu um erro inesperado durante a extração: {e}", exc_info=True)
