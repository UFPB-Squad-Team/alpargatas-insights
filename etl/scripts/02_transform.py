...
def enrich_with_coordinates(df_escolas: pd.DataFrame, df_municipios: pd.DataFrame) -> pd.DataFrame:
...
def transform_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["tipo_localizacao"] = df["tipo_localizacao"].map(localizacao_map).fillna("Desconhecida")
    
    infra_cols = [v for k, v in COLUMNS_MAP.items() if k.startswith('IN_') and v in df.columns]
    ...
    df['localizacao'] = df.apply(lambda row: {"type": "Point", "coordinates": [row['longitude'], row['latitude']]}, axis=1)
    
    def calculate_score(row):
        pontos = 0
        PONTUACAO_MAXIMA = 100
        infra = row.get('infraestrutura', {})
        if not infra.get('possui_saneamento_basico', True): pontos += 25
        if not infra.get('possui_agua_potavel', True): pontos += 20
        if not infra.get('possui_biblioteca', True): pontos += 20
        if not infra.get('possui_internet', True): pontos += 15
        if not infra.get('possui_quadra_esportes', True): pontos += 10
        if not infra.get('possui_acessibilidade_pcd', True): pontos += 10
...
def main():
    logging.info("--- INICIANDO SCRIPT DE TRANSFORMAÇÃO E GEOCODING POR CÓDIGO IBGE ---")
    ...
    if df_escolas_raw.empty or df_municipios.empty:
        logging.error("Dados de escolas ou municípios não puderam ser carregados. Encerrando.")
        return

    df_renamed = df_escolas_raw.rename(columns={k: v for k, v in COLUMNS_MAP.items() if k in df_escolas_raw.columns})
