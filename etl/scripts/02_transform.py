...
def select_and_rename_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols_to_rename = {
        old_name: new_name
        for old_name, new_name in COLUMNS_MAP_COMPLETO.items()
    ...
    existing_cols_to_select = [
        col for col in columns_to_select_original_names if col in df.columns
    ]

    if not existing_cols_to_select:
        logging.warning(
            "Nenhuma coluna relevante encontrada para seleção e renomeio. Retornando DataFrame vazio."
        )
        return pd.DataFrame()
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in COLUNAS_DE_INFRA_RENOMEADAS:
        if col in df_cleaned.columns:
            ...
            logging.warning(
                f"Coluna '{col}' não encontrada no DataFrame para conversão de tipo."
            )

    if "dependencia_adm" in df_cleaned.columns:
        dep_map = {1: "Federal", 2: "Estadual", 3: "Municipal", 4: "Privada"}
        df_cleaned["dependencia_adm"] = (
            df_cleaned["dependencia_adm"].map(dep_map).fillna("Desconhecida")
        )
    ...
    if "situacao_funcionamento" in df_cleaned.columns:
        ...
        df_cleaned["situacao_funcionamento"] = (
            df_cleaned["situacao_funcionamento"]
            .map(situacao_map)
            .fillna("Desconhecida")
        )
    ...
    if "tipo_localizacao" in df_cleaned.columns:
        ...
        df_cleaned["tipo_localizacao"] = (
            df_cleaned["tipo_localizacao"].map(localizacao_map).fillna("Desconhecida")
        )
...
def structure_data_as_documents(df: pd.DataFrame) -> pd.DataFrame:
    ...
    existing_infra_cols_for_doc = [
        col for col in COLUNAS_DE_INFRA_RENOMEADAS if col in df_structured.columns
    ]

    if existing_infra_cols_for_doc:
        df_structured["infraestrutura"] = df_structured[
            existing_infra_cols_for_doc
        ].to_dict(orient="records")
        df_structured = df_structured.drop(columns=existing_infra_cols_for_doc)
        logging.debug(
            "Colunas de infraestrutura agrupadas em 'infraestrutura' e removidas do DF principal."
        )
    else:
        logging.warning(
            "Nenhuma coluna de infraestrutura encontrada para estruturar em sub-documento."
        )
...
def load_raw_data(file_path: Path) -> pd.DataFrame:
    ...
    try:
        ...
        logging.warning(
            "O arquivo não foi encontrado, garanta que o arquivo '01_extract.py' foi executado anteriormente."
        )
        return pd.DataFrame()
    except Exception as e:
        logging.error(
            f"Erro ao carregar dados brutos de {file_path}: {e}", exc_info=True
        )
        return pd.DataFrame()
...
def main():
    ...
    if not df_raw.empty:
        ...
        logging.error(
            "Nenhum dado bruto para transformar. Encerrando o processo de transformação."
        )
