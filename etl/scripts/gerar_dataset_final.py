...
def getdtb(file_path: Path) -> pd.DataFrame:
    ...
    data = pd.read_excel(file_path, skiprows=6, usecols=['UF', 'Nome_UF', 'Código Município Completo', 'Nome_Município'])
    data.columns = ['id_uf', 'ds_uf', 'id_mundv', 'ds_mun']
    return data.drop_duplicates(subset=['id_mundv'])

def formatar_nome_super(df: pd.DataFrame, coluna: str, novo_nome: str = "ds_formatada") -> pd.DataFrame:
    logging.info(f"Aplicando formatação avançada na coluna: {coluna}")
    temp_series = df[coluna].astype(str).str.upper().apply(unidecode)
    temp_series = (temp_series.str.replace("[-.!?'`()*]", "", regex=True)
                               .str.replace("MIXING CENTER", "").str.strip().str.replace(" ", ""))
...
def processar_dados_ia(path_ia: Path) -> pd.DataFrame:
    ...
    for aba, config in config_abas.items():
        try:
            ...
            if 'iloc' in config:
                ...
                df = pd.read_excel(path_ia, sheet_name=aba, header=config['header'], usecols=config['usecols'])
    ...
    df_consolidado['nprojetos'] = pd.to_numeric(df_consolidado['nprojetos'], errors='coerce').fillna(0)
    df_consolidado['nbeneficiados'] = pd.to_numeric(df_consolidado['nbeneficiados'], errors='coerce').fillna(0)
    ...
    df_consolidado['ds_uf'] = df_consolidado['sg_uf'].map({"PB": "Paraíba", "PE": "Pernambuco", "MG": "Minas Gerais", "SP": "São Paulo", "RJ": "Rio de Janeiro"})
...
def agregar_indicadores_municipais(df_temporal: pd.DataFrame) -> pd.DataFrame:
    ...
    df_agg = df_temporal.groupby('id_mundv').agg(
...
def enriquecer_dados_escolas(path_escolas: Path, df_municipios_agg: pd.DataFrame) -> pd.DataFrame:
    ...
    df_municipios_agg['municipioIdIbge'] = pd.to_numeric(df_municipios_agg['municipioIdIbge']).astype('int64')
    df_escolas_enriquecidas = pd.merge(df_escolas, df_municipios_agg, on='municipioIdIbge', how='left')
    
    if 'municipioNome_x' in df_escolas_enriquecidas.columns:
        df_escolas_enriquecidas.rename(columns={'municipioNome_x': 'municipioNome'}, inplace=True)
    if 'municipioNome_y' in df_escolas_enriquecidas.columns:
        df_escolas_enriquecidas.drop(columns=['municipioNome_y'], inplace=True)
    
    df_escolas_enriquecidas['risco_ideb_municipio'] = 1 - (df_escolas_enriquecidas['municipioMediaIdeb2023'] / 10)
    df_escolas_enriquecidas['scoreRiscoContextualizado'] = (
        0.6 * df_escolas_enriquecidas['scoreRisco'] + 0.4 * df_escolas_enriquecidas['risco_ideb_municipio']
...
def main():
    ...
    df_ideb = pd.read_excel(data_path_raw / "divulgacao_anos_iniciais_municipios_2023.xlsx", 
                          skiprows=9, usecols=['CO_MUNICIPIO', 'REDE'] + lista_ideb, na_values=['-', '--'])
    ...
    df_temporal = pd.merge(df_ia_formatado, df_dtb_formatado, on=["ds_formatada", "ds_uf"], how="inner")
    ...
    df_temporal.to_parquet(base_path / "data/processed/dados_temporais_para_auditoria.parquet", index=False)
    ...
    caminho_final = base_path / "data" / "processed" / "ENTREGAVEL_escolas_enriquecidas.parquet"
    df_final.to_parquet(caminho_final, index=False)
    logging.info(f" O processo foi concluido com sucesso: O entregável final foi salvo em: {caminho_final} ---")
