...
def calculate_basic_infra_percent(df_paraiba, infra_basicas):
    ...
    for codigo, nome in infra_basicas.items():
        ...
        porcentagem_escolas_sem_infra_basica.append(
            {"Item": nome, "Faltam (%)": percentual_sem_infra, "Total": total}
        )

    df_resultado_porcentagem_escolas_sem_infra_basica = pd.DataFrame(
        porcentagem_escolas_sem_infra_basica
    ).sort_values("Faltam (%)", ascending=False)
