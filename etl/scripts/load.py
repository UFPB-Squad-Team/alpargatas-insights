...
def prepare_data_for_mongo(df: pd.DataFrame, column_name: str) -> pd.DataFrame:
    ...
    logging.info(
        f"Convertendo a coluna {column_name} para a formatação ideal para o MongoDB"
    )
...
def prepare_bulk_upsert_operations(df: pd.DataFrame) -> List[UpdateOne]:
    ...
    operations = [
        UpdateOne({"escola_id_inep": doc["escola_id_inep"]}, {"$set": doc}, upsert=True)
...
def load_to_mongo(
    operations: List[UpdateOne], db_name: str, collection_name: str, uri: str
):
    ...
    try:
        ...
        if not operations:
            logging.warning(
                "A lista de operações upsert está vazia. Nenhum dado será inserido na coleção."
            )
            return
