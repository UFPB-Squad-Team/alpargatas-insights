    ...
    <div className="bg-brand-background p-6 rounded-2xl shadow-sm border border-gray-200 flex-1 min-w-[200px] flex flex-col justify-between">
      <div>
        ...
        <p
          ...
          font-bold text-brand-primary my-2
          break-words
          capitalize
        "
        >
          {value}
          {IconSecondary && (
            <IconSecondary className="inline-block ml-2 h-6 w-6 text-brand-accent-dark" />
          )}
        </p>
      </div>
      <p className="text-xs font-semibold text-brand-secondary mt-auto">{description}</p>
    </div>
  );
};
